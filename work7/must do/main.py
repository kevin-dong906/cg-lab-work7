...
def draw_mesh(ax, vertices, faces, joints=None, vertex_scalar=None, face_colors=None, title="", elev=12, azim=108):
    ...
    mesh = Poly3DCollection(plot_vertices[faces], facecolors=face_colors, linewidths=0.03, edgecolors=(0.0, 0.0, 0.0, 0.05))
    ax.add_collection3d(mesh)
    if joints is not None:
        ax.scatter(plot_joints[:, 0], plot_joints[:, 1], plot_joints[:, 2], c="white", s=12, depthshade=False, edgecolors="black", linewidths=0.3)
...
def save_comparison_grid(path, data_dict, faces):
    ...
    draw_mesh(ax1, data_dict["v_template"], faces, joints=data_dict["J_template"], vertex_scalar=data_dict["weight_scalar"], title="(a) Template + LBS Weights")
    ax2 = fig.add_subplot(222, projection='3d')
    draw_mesh(ax2, data_dict["v_shaped"], faces, joints=data_dict["J_shaped"], title="(b) Shape Blend + Joint Regression")
    ax3 = fig.add_subplot(223, projection='3d')
    draw_mesh(ax3, data_dict["v_posed"], faces, joints=data_dict["J_shaped"], vertex_scalar=data_dict["pose_offset_norm"], title="(c) Pose Blend Shapes")
    ax4 = fig.add_subplot(224, projection='3d')
    draw_mesh(ax4, data_dict["verts"], faces, joints=data_dict["J_transformed"], title="(d) Final LBS Result")
    fig.tight_layout()
    fig.savefig(path, dpi=220, bbox_inches="tight")
    plt.close(fig)
...
def save_all_joint_weights_figure(path, vertices, faces, joints, lbs_weights):
    ...
    ax = fig.add_subplot(111, projection='3d')
    draw_mesh(ax, vertices, faces, joints=joints, face_colors=get_face_colors_from_joint_weights(lbs_weights, faces), title="All Joint LBS Weights")
    fig.tight_layout()
    fig.savefig(path, dpi=220, bbox_inches="tight")
    plt.close(fig)
...
def build_demo_pose(device, dtype):
    ...
    joint_names = {"left_hip":1,"right_hip":2,"left_knee":4,"right_knee":5,"left_shoulder":16,"right_shoulder":17,"left_elbow":18,"right_elbow":19}
...
def compute_manual_lbs(model, betas, global_orient, body_pose):
    ...
    return {"v_template":v_template,"J_template":J_template,"v_shaped":v_shaped,"J_shaped":J,"pose_offsets":pose_offsets,"v_posed":v_posed,"J_transformed":J_transformed,"verts":verts}
...
def main():
    ...
    betas = build_demo_shape(device, dtype)
    global_orient, body_pose = build_demo_pose(device, dtype)
    data = compute_manual_lbs(model, betas, global_orient, body_pose)
    mean_err, max_err = compare_with_official_forward(model, betas, global_orient, body_pose, data["verts"])
    joint_id = 18
    weight_scalar = to_numpy(model.lbs_weights[:, joint_id])
    pose_offset_norm = np.linalg.norm(to_numpy(data["pose_offsets"][0]), axis=1)
    
    save_single_figure(os.path.join(out_dir, "stage_a_template_weights.png"), to_numpy(data["v_template"][0]), faces, joints=to_numpy(data["J_template"][0]), vertex_scalar=weight_scalar, title=f"(a) Template Mesh + Weight of Joint {joint_id}")
    save_single_figure(os.path.join(out_dir, "stage_b_shaped_joints.png"), to_numpy(data["v_shaped"][0]), faces, joints=to_numpy(data["J_shaped"][0]), title="(b) Shape Blend + Joint Regression")
    save_single_figure(os.path.join(out_dir, "stage_c_pose_offsets.png"), to_numpy(data["v_posed"][0]), faces, joints=to_numpy(data["J_shaped"][0]), vertex_scalar=pose_offset_norm, title="(c) Pose Blend Shapes (colored by |pose_offsets|)")
    save_single_figure(os.path.join(out_dir, "stage_d_lbs_result.png"), to_numpy(data["verts"][0]), faces, joints=to_numpy(data["J_transformed"][0]), title="(d) Final LBS Result")
    
    grid_dict = {"v_template":to_numpy(data["v_template"][0]),"J_template":to_numpy(data["J_template"][0]),"v_shaped":to_numpy(data["v_shaped"][0]),"J_shaped":to_numpy(data["J_shaped"][0]),"v_posed":to_numpy(data["v_posed"][0]),"verts":to_numpy(data["verts"][0]),"J_transformed":to_numpy(data["J_transformed"][0]),"weight_scalar":weight_scalar,"pose_offset_norm":pose_offset_norm}
    save_comparison_grid(os.path.join(out_dir, "comparison_grid.png"), grid_dict, faces)
    save_all_joint_weights_figure(os.path.join(out_dir, "all_joint_weights.png"), to_numpy(data["v_template"][0]), faces, to_numpy(data["J_template"][0]), to_numpy(model.lbs_weights))
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"num_betas: 10\n")
        f.write(f"visualized_joint_id: 18\n")
